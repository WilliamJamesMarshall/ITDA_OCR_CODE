...
- 이 컴퓨터의 유일한 활성 clone은 `C:\ITDA_OCR_CODE`다.
...
- Codex 데스크톱 프로젝트와 현재 세션이 사용하는 `C:\ITDA_OCR_CODE`를 정본으로 확정했다.
- 두 clone의 `origin`과 기준 HEAD가 동일함을 확인했다.
- 원격 최신 `main`의 `Harness_README.md`, `.git-session.json`, `scripts/git-session-manager.mjs`, 이 통합 문서를 정본에 fast-forward 방식으로 통합했다.
- `images.zip`과 `상품사진입니다\`를 정본인 `C:\ITDA_OCR_CODE`로 이동했다.
...
- junction은 필요하지 않다. 노트북 콘솔과 iPad Remote가 모두 같은 Codex 프로젝트와 실제 경로를 사용한다.
...
1. Current repository가 `ITDA_OCR_CODE`이고 로컬 경로가 `C:\ITDA_OCR_CODE`인지 확인한다.
2. 하네스 도입 커밋에서는 다음 네 항목을 검토한다.
   - `Harness_README.md`
   - `.git-session.json`
   - `scripts/git-session-manager.mjs`
   - `docs/repository-unification-plan.md`
3. `images.zip`과 `상품사진입니다\`가 Changes에 나타나지 않는지 확인한다.
4. 변경 목적에 맞는 제목으로 `main`에 커밋한다.
5. `origin`에 push한 뒤 원격 `main`의 최신 커밋을 확인한다.
