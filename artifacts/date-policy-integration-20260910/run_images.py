...
parser.add_argument('--ocr-cache', type=Path, action='append', default=[],
                    help='Completed run directory; exact OCR inputs only. Cache timing is not E2E.')
...
cache = {}
meta['cache_sources'] = []
meta['measurement'] = ('adaptive exact-input OCR cache + real model misses; NOT E2E timing'
                       if args.ocr_cache else 'fresh image execution')
for directory in args.ocr_cache:
    directory = directory.resolve()
    runtime_file, log_file = directory/'runtime.json', directory/'predictions.jsonl'
    cached_meta = json.loads(runtime_file.read_text(encoding='utf-8'))
    assert cached_meta['config'] == json.loads(json.dumps(meta['config'],default=str))
    assert cached_meta['packages'] == meta['packages']
    assert cached_meta['weights_sha256'] == meta['weights_sha256']
    pipeline_path = str(ROOT/'src/pipeline.py')
    assert cached_meta['code_sha256'][pipeline_path] == meta['code_sha256'][pipeline_path]
    assert all(sha(Path(name))==digest for name,digest in cached_meta['images_sha256'].items())
    cached_rows = [json.loads(raw) for raw in log_file.read_text(encoding='utf-8').splitlines()]
    assert len(cached_rows)==len(cached_meta['images_sha256'])
    assert {row['image_id'] for row in cached_rows} == {Path(name).stem for name in cached_meta['images_sha256']}
    for record in cached_rows:
        for event in record['events']:
            key = (event['detector'],event['variant'],tuple(event['shape']),event['input_sha256'])
            if key in cache:
                assert cache[key]==event['lines'], 'Conflicting OCR evidence for identical input'
            cache[key] = event['lines']
    meta['cache_sources'].append({'directory':str(directory),'runtime_sha256':sha(runtime_file),
                                  'predictions_sha256':sha(log_file)})
...
class TraceBackend:
    def __init__(self):
        self.events = []
        self.cache_hits = self.fresh_calls = 0
    def recognize(self, image, *, detector, variant):
        begin = time.perf_counter()
        input_hash = hashlib.sha256(image.tobytes()).hexdigest()
        key = (detector,variant,tuple(image.shape),input_hash)
        cached = cache.get(key)
        if cached is not None:
            lines = [p.OCRLine(**line) for line in cached]
            self.cache_hits += 1
        else:
            self.fresh_calls += 1
        ...
        self.events.append({'detector':detector,'variant':variant,'shape':list(image.shape),
                            'input_sha256':input_hash,'cache_hit':cached is not None,
...
meta['cache_hits'], meta['fresh_calls'] = backend.cache_hits, backend.fresh_calls
