...
original=openpyxl.load_workbook(Path('C:/ITDA_OCR_WORKTREES/lee-hoyeon/labels/validation_003353_003716_manual.xlsx'),data_only=False)
...
for stage in ('baseline','p3'):
    path=LABELS/f'validation_003353_003716_{stage}_20260910.xlsx'
    ...
    if stage=='p3':
        previous=openpyxl.load_workbook(OUT/'previous_results'/path.name,data_only=True)['검수 정답지']
        for row in range(1,388):
            for col in range(1,8):
                assert blank(previous.cell(row,col).value)==blank(cached.cell(row,col).value),('Improved result changed outside notes',row,col)
        results[stage]['previous_columns_A_to_G_unchanged']=True
...
assert baseline_meta['variant']['MIN_YEAR']==1 and baseline_meta['variant']['MAX_YEAR']==2035
