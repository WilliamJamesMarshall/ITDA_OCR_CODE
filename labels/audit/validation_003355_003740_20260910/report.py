...
def exact(s):
    a=scores[s]
    return f'{a["correct"]}/386 ({pct(a["accuracy"])})'
...
'',
f'500장 선형 환산은 이미지 처리 구간 기준 각각 {baseline["runtime"]["loop_wall_seconds"]*500/386:.0f}초, {improved["runtime"]["loop_wall_seconds"]*500/386:.0f}초다. 이는 500장 실측이나 공식 평가 환경 점수가 아니다.',
...
'',
f'시간 비교 통제: 모든 이미지의 첫 mobile 원본 패스 입력 해시는 동일했다. OCR 문자열 출력도 {paired["same_text_count"]}/386건이 동일했다. 그럼에도 동일 패스의 p3/baseline 시간비 중앙값은 {paired["median_p3_over_baseline_ratio"]:.2f}였다. 실행 시점에 따른 시간 변동이 관찰되므로 위의 속도 차이를 코드 변경만의 순수 효과로 단정할 수 없다. 정밀한 속도 추정은 교차 순서 반복 측정이 필요하다.',
'',
'현재 코드의 MIN_YEAR=2024, MAX_YEAR=2035 범위에 들어가는 정답은 완전 날짜 4건뿐이다. 부분 날짜 25건도 기존 출력 계약으로는 표현할 수 없다. NONE 정답 7건을 더해도 이 데이터에서 기존 코드의 이론적 상한은 11/386(2.85%)다. 이 낮은 점수를 OCR 인식기 자체의 일반 정확도로 해석해서는 안 된다.',
'',
'## 2. 실행 우선순위별 후처리 비교',
'',
...
for i in ('003499','003575','003414','003510','003542','003566','003739'):
    text.append(f'| {i} | {a[i]["truth"]} | {a[i]["prediction"]} | {b[i]["prediction"]} |')
text += ['',
'baseline 원시 로그에서 확인한 원인:',
'',
'- 003510: 원본의 날짜 문자열은 `EXP17 2 2022`로 오인식됐다. ROI 1은 전화번호가 포함된 주소, ROI 2는 `11200`을 읽었다. 현재 ROI 기준이 높은 OCR 점수와 구분자에 치우쳐 실제 날짜 영역을 놓치는 사례다. CLAHE에서는 `EXP:07. 12.2022`를 읽었지만 날짜 순서와 후보 선택 문제가 남았다.',
'- 003566: 원본·CLAHE·복구 검출에서 정답 날짜를 읽지 못했다. 이 경우 문자열의 MDY 해석 규칙만 추가해도 정답을 얻을 수 없다.',
'- 003575: CLAHE OCR 출력이 `15/07/2021`이었다. 정답 `2021-07-05`와 다른 숫자가 인식돼 파서 규칙만으로 확정 교정할 수 없다.',
'- 003739: `해동일로부터의 유통기한 2021.04.27`과 `유통기한:02/09/2021`이 모두 인식됐다. 날짜를 읽는 문제와 별개로 적용해야 할 기한의 역할을 선택해야 한다. 현재 단순 부정 키워드 규칙은 `해동일로부터의`에 있는 `부터`에도 불이익을 줄 수 있다.',
'']
...
'## 5. 측정 조건과 원본 보존',
'',
'- 정답: 사용자가 수정한 manual.xlsx의 `검수 정답지!C2:C387`. 원본 CSV와 inspect.ndjson은 정답으로 사용하지 않았다.',
'- 판독 입력: 003355.jpg부터 003740.jpg까지 이미지 386장만 사용했다. metadata/annotations.json, 이전 라벨 매핑, 이미지 ID별 정답 예외 규칙은 사용하지 않았다.',
...
'- baseline은 20260909 audit의 완료된 386장 실행 기록을 재사용했다. 원본 3개·이미지 386장·운영 코드 2개의 해시와 정답 스냅샷을 확인했다. 앞선 p3 282장 중단 기록은 최종 채점과 시간에서 제외하고 20260910에 386장 전부를 새 프로세스로 실행했다.',
...
'- 이미 알려진 오류 유형을 바탕으로 설계한 개발 검증이다. 독립 블라인드 평가가 아니며 이 386장의 정확도를 일반화 성능으로 주장하지 않는다. 정답 채점 후 실험 규칙을 재조정하지 않았다.',
'- 앞선 384/386(99.48%) 판독은 주석 정보와 시각 검토를 사용한 작업이었다. 이번 이미지 단독 CPU OCR과 입력 조건이 다르므로 동일 방식의 재현 성능으로 볼 수 없다.',
'- 원본 CSV·XLSX·inspect.ndjson 세 파일의 SHA-256이 실행 전후 동일함을 확인했다. 운영 코드와 가중치도 수정하지 않았다.',
'',
'## 6. 저장 파일',
'',
'baseline과 p3의 새 XLSX는 원본 시트명·8열·정답·True/False 비교 수식을 유지했다. 각 단계의 CSV도 같은 8열을 사용한다. 실행 로그·측정 환경·채점 근거·테스트 결과·재현 코드는 labels/audit/validation_003355_003740_20260910에 있다.',
'',
'XLSX 생성 도구가 가져오기 과정에서 누락한 공유 수식의 후속 행은 새 파일에서만 원본과 동일한 수식으로 복원했다. 계산 결과와 CSV를 386행 전수 대조했고, 글꼴·채우기·테두리·정렬·표시 형식·보호 속성의 원본 대비 차이는 0건이었다. 표·고정 창도 보존했다. 각 새 XLSX의 inspect.ndjson에는 저장 파일 해시와 값·수식을 기록했다. 수식은 Artifact Tool에서 재계산하고 저장 파일은 별도 읽기 도구로 검산했으며 실제 Microsoft Excel 앱의 재계산은 시험하지 않았다.',
'']
target=LABELS/'validation_003355_003740_report_20260910.md'
