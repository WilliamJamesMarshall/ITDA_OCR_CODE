...
const labels=path.join(dir,'outputs');
const realLabels='C:/ITDA_OCR_WORKTREES/lee-hoyeon/labels';
const source=path.join(realLabels,'validation_003355_003740_manual.xlsx');
...
for(const stage of ['baseline','p3']){
  ...
  // Import omits followers of the source shared formula. Restore the exact
  // source pattern in the new workbook only; independently verified for all rows.
  const formulas=Array.from({length:386},(_,i)=>{
    const r=i+2;
    return [`=IF(OR(B${r}="",C${r}=""),"",IF(B${r}=C${r},"True","False"))`];
  });
  const rows=data[stage].rows;
  if(rows.length!==386)throw Error('Expected 386 rows');
  rows.forEach((r,i)=>{
  ...
  sheet.getRange('D2:D387').formulas=formulas;
  ...
  for(let i=0;i<386;i++){
    if(actual[i+1][3]!==rows[i][3])throw Error(`Comparison mismatch ${stage} row ${i+2}: ${JSON.stringify({actual:actual[i+1][3],expected:rows[i][3],formula:formulas[i],inputs:actual[i+1].slice(0,3)})}`);
  ...
  const target=path.join(labels,`validation_003355_003740_${stage}_20260910.xlsx`);
  await (await SpreadsheetFile.exportXlsx(wb)).save(target);
  const sha=crypto.createHash('sha256').update(await fs.readFile(target)).digest('hex');
  const allFormulas=sheet.getRange('A1:H387').formulas;
  const inspection=[{kind:'workbook',source:path.join(realLabels,path.basename(target)),sha256:sha,sheets:1,rows:387,dataRows:386,columns:8},
    {kind:'sheet',name:sheet.name,range:'A1:H387'},
    ...actual.map((values,i)=>({kind:'row',sheet:sheet.name,row:i+1,values,formulas:allFormulas[i]}))];
  await fs.writeFile(target+'.inspect.ndjson',inspection.map(r=>JSON.stringify(r)).join('\n')+'\n');
