...
assert improved['correct']==284 and all(v['total']==386 for v in scores.values())
def exact(stage):
    s=scores[stage]
    return f'{s["correct"]}/386 ({s["accuracy"]*100:.2f}%)'
...
lines=[
'# 소비기한 OCR 386장 검증 결과 — 연도 하한 제거 기준으로 수정',
'',
...
'',
'기준군은 이번에 이미지 386장을 처음부터 다시 실행했다. 개선안의 예측값과 처리시간은 앞서 완료한 386장 실행 기록을 검증하여 재사용했으며 새로 실행한 수치가 아니다. 개선안 XLSX·CSV에는 새 기준 대비 개선/회귀/양쪽 정답/양쪽 오답을 비고에 반영했다.',
'',
'## 1. 이번에 변경한 기준',
'',
...
'',
f'관측된 개선안/기준군 이미지 처리시간 비는 {ratio:.3f}다(1 미만이면 개선안이 짧음). 서로 다른 시점의 단일 실행이므로 코드 변경만의 속도 효과로 단정하지 않는다. 첫 원본 OCR 입력은 386장 모두 동일했고 문자열 출력은 {paired["same_text_count"]}/386장, 좌표·신뢰도까지 포함한 출력은 {paired["same_full_ocr_lines_count"]}/386장이 같았다. 같은 첫 OCR 패스의 개선안/기준군 시간비 중앙값은 {paired["median_p3_over_baseline_ratio"]:.3f}였다.',
'',
...
'',
f'500장 선형 환산(이미지 처리 구간)은 기준군 {baseline["runtime"]["loop_wall_seconds"]*500/386:.0f}초, 개선안 {improved["runtime"]["loop_wall_seconds"]*500/386:.0f}초다. 실제 500장이나 공식 4-vCPU 환경을 측정한 것이 아니다. 프로젝트의 2,400초 한도·1,800초 내부 목표 통과 여부를 이 값으로 확정하지 않는다.',
'',
'## 3. 새 기준 대비 정답 증감',
'',
...
for key in ('003499','003575','003414','003510','003542','003566','003739'):
    lines.append(f'| {key} | {a[key]["truth"]} | {a[key]["prediction"]} | {b[key]["prediction"]} |')
lines += ['',
'## 6. 적용 판단과 한계',
'',
'- 기존의 10/386(2.59%)·초기화 포함 3,484.3초는 연도 제한이 켜진 과거 기록이다. 이제 주 비교 기준에서 제외했으며 이전 파일은 백업으로 보관했다. 이전에 제시했던 +70.98%p와 약 2배 속도 차이는 이번 기준군에 적용하지 않는다.',
...
'- 정답은 사용자 수정본 manual.xlsx의 C2:C387이며 문자열 전체가 같아야 정답이다. 부분 날짜는 NONE-MM-DD 또는 YYYY-MM-NONE, 유효 날짜 없음은 NONE으로 비교한다. 실행 예외는 정답이 NONE이어도 실패 처리한다.',
...
'',
'새 실행·채점·비교 근거는 labels/audit/validation_003355_003740_20260910_year_unrestricted에 저장한다. 그 아래 previous_results에 교체 전 결과 11개를 보관한다. 이전 audit 디렉터리의 원시 실행 기록은 역사 기록으로 남긴다.',
'',
...
'']
target=OUT/'outputs/validation_003355_003740_report_20260910.md'
