...
for stage in stages:
    ...
    for source in truth:
        ...
        if stage=='baseline':
            ...
            before=detail['baseline']['records'][len(rows)]['correct']
            comparison='기준 대비 개선' if equal and not before else '기준 대비 회귀' if before and not equal else '양쪽 정답' if equal else '양쪽 오답'
            note+='; '+comparison
        if 'replay' in stage:
    ...
    filename = f'validation_003355_003718_{stage}_20260910.csv'
...
assert set(result)==set(stages), 'Every stage must have 386 complete records'
