...
for i in ('003493','003567','003411','003504','003534','003558',):
    text.append(f'| {i} | {a[i]["truth"]} | {a[i]["prediction"]} | {b[i]["prediction"]} |')
text += ['',
'baseline 원시 로그에서 확인한 원인:',
'',
'- 003504: 원본의 날짜 문자열은 `EXP17 2 2022`로 오인식됐다. ROI 1은 전화번호가 포함된 주소, ROI 2는 `11200`을 읽었다. 현재 ROI 기준이 높은 OCR 점수와 구분자에 치우쳐 실제 날짜 영역을 놓치는 사례다. CLAHE에서는 `EXP:07. 12.2022`를 읽었지만 날짜 순서와 후보 선택 문제가 남았다.',
'- 003558: 원본·CLAHE·복구 검출에서 정답 날짜를 읽지 못했다. 이 경우 문자열의 MDY 해석 규칙만 추가해도 정답을 얻을 수 없다.',
'- 003567: CLAHE OCR 출력이 `15/07/2021`이었다. 정답 `2021-07-05`와 다른 숫자가 인식돼 파서 규칙만으로 확정 교정할 수 없다.',
'']
...
'- 판독 입력: 003355.jpg부터 003718.jpg까지 이미지 364장만 사용했다. metadata/annotations.json, 이전 라벨 매핑, 이미지 ID별 정답 예외 규칙은 사용하지 않았다.',
...
'## 6. 저장 파일',
'',
'baseline과 p3의 새 XLSX는 원본 시트명·8열·정답·True/False 비교 수식을 유지했다. 각 단계의 CSV도 같은 8열을 사용한다. 실행 로그·측정 환경·채점 근거·테스트 결과·재현 코드는 labels/audit/validation_003355_003718_20260909에 있다.',
'']
target=LABELS/'validation_003355_003718_report_20260909.md'
