...
새 PC에서 학습 자료를 복원할 때는 외부 저장소 백업과 이미지 경로를 함께 복원해야 합니다. 제출 추론에는 학습 자료나 이 호환 연결이 필요하지 않습니다.
현재 규칙은 [원본 6회·4단계 계획](../protocol/grouped_6_rounds.md)이며 상태를 조회하여 미완료 작업부터 이어갑니다.
실제 학습은 기존 승인과 release gate를 확인합니다. [새 세션 프롬프트](grouped_session_prompts.md)를 참고하세요.
