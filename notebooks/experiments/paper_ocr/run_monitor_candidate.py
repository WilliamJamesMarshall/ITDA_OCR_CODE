...
LEGACY = Path('C:/ITDA_OCR_WORKSPACE/grouped-8-rounds-v2')
DEV = LEGACY/'paper-ocr-20260914'
...
def main():
    ...
    if (LEGACY/'locks/execution.lock').exists():
        raise RuntimeError('A legacy execution lock exists; do not overlap another run')
    with exclusive(BASE/'locks/execution.lock'):
        ...
        for number in (2,3):
            ...
            original_rows = csv_read(manifest)
            old_rows = csv_read(LEGACY/f'test_round_{number:02}.csv')
            if [r['image_id'] for r in original_rows] != [r['image_id'] for r in old_rows]:
                raise ValueError('Migration changed the requested image set or order')
            for current, old in zip(original_rows, old_rows):
                if Path(current['image_path']).resolve().parent != (ROOT/'학습대상데이터').resolve():
                    raise ValueError('Only the original-image directory can supply new runs')
                if digest(current['image_path']) != digest(old['image_path']):
                    raise ValueError('Migration image bytes differ: '+current['image_id'])
            input_equivalence[str(number)] = dict(images=len(original_rows), identical_bytes=True,
                                                  old_manifest=evidence(LEGACY/f'test_round_{number:02}.csv'))
            if args.smoke:
        ...
        job = dict(parent_pid=os.getpid(), authorization=evidence(DEV/'authorization.json'),
            ...
            execution_policy='grouped-6-originals-v1', input_equivalence=input_equivalence,
