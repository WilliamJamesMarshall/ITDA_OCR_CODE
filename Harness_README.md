...
| 5 | `final_date` | `YYYY-MM-DD`, 기존 부분 날짜 `NONE-MM-DD`/`YYYY-MM-NONE`, 또는 `NONE` |

- 한 이미지당 정확히 한 행을 생성한다.
- 날짜 열은 문자열로 유지해 앞의 0을 보존한다.
- 결정적인 정렬 기준을 사용한다.
- CSV 인덱스를 저장하지 않는다.
- 2026-09-11 사용자가 전달한 운영진 출력 스키마 정정에 따라 전체 미인식 시 `year,month,day`는 각각 `NONE`, `final_date`는 `NONE`으로 둔다. 과거 `NONE-NONE-NONE`은 의미 비교에서만 호환한다.
...
| 판독 불가·날짜 없음 | `year,month,day`는 각각 `NONE`, `final_date`는 `NONE`으로 저장 |
